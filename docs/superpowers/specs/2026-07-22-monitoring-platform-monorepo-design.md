...
2. Create immutable checkpoint tags in each existing repository.
3. Push archival refs before any path rewrite.
...
- Tag current GPU live, GPU development, and Storage active states.
- Inventory ignored runtime data and secrets.
...
- Make no behavior or visual changes.

### Phase 2: normalize builds
...
- Add secret and generated-data checks.
- Require successful affected checks on main.

### Phase 4: services and releases

- Add systemd templates and atomic deployment scripts.
- Install the deploy-only runner and narrowly scoped restart permission.
- Deploy to development ports and exercise rollback.
