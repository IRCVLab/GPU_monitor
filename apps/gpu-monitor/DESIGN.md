...
- Expanded System shows interpreted recent-10-second CPU/I/O delay percentages before quieter expert diagnostics. Raw 1/5/15-minute load, runnable tasks, logical CPU count, I/O full, and blocked tasks remain visible only in that diagnostic layer. PSI cumulative `total` is not collected or displayed. Essential interpretation never depends on hover; detailed PSI values may add supplementary accessible tooltips using visible copy, `role="tooltip"`, and `aria-describedby`. Tooltips open on hover/focus, dismiss on `Escape`, remain visible while trigger or tooltip is hovered, close after focus/pointer leaves both, contain no interactive content, and do not use native `title` as the explanation path.
