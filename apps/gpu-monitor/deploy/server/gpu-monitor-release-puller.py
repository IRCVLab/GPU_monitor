#!/usr/bin/env python3
"""Outbound-only gpu-live release puller.

Polls public GitHub API evidence for the current main SHA, authorizes it with the
canonical installed authorizer, builds as an isolated builder user, then hands the
validated artifact to the existing local immutable activator.
"""
...
class Config:
    ...
    state_dir: Path = Path("/var/lib/gpu-monitor/puller")
    work_dir: Path = Path("/var/lib/gpu-monitor/builder/work")
    authorizer: Path = Path("/usr/local/libexec/authorize_gpu_release.py")
    activate: Path = Path("/usr/local/libexec/activate-release.sh")
    build_script: str = "apps/gpu-monitor/deploy/build-release.sh"
    builder_user: str = "gpu-monitor-builder"
    deploy_user: str = "gpu-deploy-live"
    node_prefix: Path = Path("/opt/gpu-monitor/node")
...
def default_get_json(path: str) -> object:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "gpu-monitor-release-puller",
...
def write_state_sha(config: Config, sha: str) -> None:
    ...
    tmp = config.state_dir / f".current-live-sha.{os.getpid()}.tmp"
    with tmp.open("w", encoding="utf-8") as handle:
        handle.write(sha + "\n")
        handle.flush()
        os.fsync(handle.fileno())
    os.replace(tmp, config.state_dir / "current-live-sha")
    fd = os.open(config.state_dir, os.O_RDONLY | getattr(os, "O_DIRECTORY", 0))
    try:
        os.fsync(fd)
    finally:
        os.close(fd)
...
def write_failed_release(config: Config, sha: str, failures: int, now: float) -> None:
    ...
    if (
        failures < 1
        or config.failure_backoff_base_seconds < 1
        or config.failure_backoff_max_seconds < config.failure_backoff_base_seconds
    ):
        raise PullError("failure backoff configuration is invalid")
    delay = min(
        config.failure_backoff_base_seconds * (2 ** min(failures - 1, 30)),
    ...
    config.state_dir.mkdir(parents=True, exist_ok=True)
    path = failed_release_path(config)
    tmp = config.state_dir / f".failed-release.{os.getpid()}.tmp"
    with tmp.open("w", encoding="utf-8") as handle:
...
def authorize(config: Config, sha: str, workflow_run: dict[str, object], check_runs: list[dict[str, object]], run_command=default_run_command) -> None:
    with tempfile.TemporaryDirectory(prefix="gpu-puller-auth.", dir=str(config.state_dir)) as tmpdir:
...
def builder_command(config: Config, *args: str) -> list[str]:
    # Keep the builder isolated from root/deploy environment and specifically
    # from /etc/gpu-monitor/live.env; only the managed Node prefix is exposed.
    return [
        "runuser", "-u", config.builder_user, "--",
        "env", "-i",  # env -i: scrub root/deploy env before builder commands
        "HOME=/var/lib/gpu-monitor/builder",
...
def build_release(config: Config, checkout: Path, sha: str, run_command=default_run_command) -> Path:
    ...
    rm_result = run_command(builder_command(config, "rm", "-rf", str(outdir)), timeout=config.timeout_seconds)
    if rm_result.returncode != 0:
        raise PullError("builder could not clean output directory")
    mkdir_result = run_command(builder_command(config, "mkdir", "-p", str(outdir)), timeout=config.timeout_seconds)
...
def validate_artifact(outdir: Path, sha: str) -> tuple[Path, str]:
    artifact = outdir / f"gpu-monitor-{sha}.tar.gz"
    checksum = outdir / f"gpu-monitor-{sha}.sha256"
    manifest = outdir / "release-manifest.json"
    if not artifact.is_file() or not checksum.is_file() or not manifest.is_file():
        raise PullError("release output is incomplete")
    digest = hashlib.sha256(artifact.read_bytes()).hexdigest()
    if DIGEST_RE.fullmatch(digest) is None:
        raise PullError("artifact digest is malformed")
    checksum_text = checksum.read_text(encoding="utf-8").strip().split()
    if not checksum_text or checksum_text[0] != digest:
        raise PullError("artifact checksum mismatch")
    try:
        manifest_payload = json.loads(manifest.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise PullError("release manifest is malformed") from error
    expected = {"application": "gpu-monitor", "artifact": artifact.name, "git_sha": sha, "schema": 1, "sha256": digest}
    if manifest_payload != expected:
        raise PullError("release manifest does not match artifact")
    return artifact, digest


def upload_artifact(config: Config, sha: str, digest: str, artifact: Path, run_command=default_run_command) -> None:
    argv = [
        "runuser", "-u", config.deploy_user, "--",
        str(config.activate), "upload", "live", sha, digest,
    ]
...
def discard_artifact(config: Config, sha: str, digest: str, run_command=default_run_command) -> None:
    argv = [
        "runuser", "-u", config.deploy_user, "--",
        str(config.activate), "discard", "live", sha, digest,
    ]
...
def live_status(config: Config, run_command=default_run_command) -> dict[str, str]:
    argv = [
        "runuser", "-u", config.deploy_user, "--",
        str(config.activate), "status", "live",
    ]
    result = run_command(argv, timeout=config.timeout_seconds)
    if result.returncode != 0:
        raise PullError(f"activation command failed: {' '.join(argv)}")
...
def activate_uploaded(config: Config, sha: str, digest: str, run_command=default_run_command) -> None:
    argv = [
        "runuser", "-u", config.deploy_user, "--",
        str(config.activate), "activate", "live", sha, digest,
    ]
    result = run_command(argv, timeout=config.timeout_seconds)
    if result.returncode != 0:
        raise PullError(f"activation command failed: {' '.join(argv)}")
...
def parse_args(argv: list[str]) -> Config:
    ...
    parser.add_argument("--deploy-user", default=Config.deploy_user)
    ...
    parser.add_argument(
        "--failure-backoff-base-seconds",
        type=int,
        default=Config.failure_backoff_base_seconds,
    )
    parser.add_argument(
        "--failure-backoff-max-seconds",
        type=int,
        default=Config.failure_backoff_max_seconds,
    )
...
def main(argv: list[str] | None = None) -> int:
    try:
        result = run_once(parse_args(sys.argv[1:] if argv is None else argv))
        print(result)
        return 0
