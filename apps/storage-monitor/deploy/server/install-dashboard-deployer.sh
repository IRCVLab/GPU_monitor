...
start_candidate_topology() {
  ...
  if [[ "$DRY_RUN" == 1 ]]; then
    run_cmd env PYTHONDONTWRITEBYTECODE=1 STORAGE_VIZ_ROOT="$CANDIDATE_RELEASE" STORAGE_VIZ_BIND=127.0.0.1 STORAGE_VIZ_PORT=18088 STORAGE_VIZ_INVENTORY="$inventory" STORAGE_VIZ_DATA_DIR="$CANDIDATE_TMP/data" STORAGE_VIZ_STATE_DIR="$CANDIDATE_TMP/state" STORAGE_VIZ_PREFLIGHT_BACKEND=1 STORAGE_VIZ_TRUSTED_PROXY=1 STORAGE_VIZ_ALLOWED_ORIGINS="$public_origin" STORAGE_VIZ_OPERATOR_ALLOWLIST="$operator" STORAGE_VIZ_SESSION_COOKIE_SECURE="$cookie_secure" "$PYTHON" "$CANDIDATE_RELEASE/viewer/serve.py"
    run_cmd env PYTHONDONTWRITEBYTECODE=1 STORAGE_VIZ_PROXY_BIND=127.0.0.1 STORAGE_VIZ_PROXY_PORT=1505 STORAGE_VIZ_PROXY_UPSTREAM_HOST=127.0.0.1 STORAGE_VIZ_PROXY_UPSTREAM_PORT=18088 STORAGE_VIZ_PROXY_OPERATOR="$operator" STORAGE_VIZ_PROXY_PUBLIC_ORIGIN="$public_origin" "$PYTHON" "$CANDIDATE_RELEASE/deploy/direct_proxy.py"
    return
  fi
  PYTHONDONTWRITEBYTECODE=1 STORAGE_VIZ_ROOT="$CANDIDATE_RELEASE" STORAGE_VIZ_BIND=127.0.0.1 STORAGE_VIZ_PORT=18088 STORAGE_VIZ_INVENTORY="$inventory" STORAGE_VIZ_DATA_DIR="$CANDIDATE_TMP/data" STORAGE_VIZ_STATE_DIR="$CANDIDATE_TMP/state" STORAGE_VIZ_PREFLIGHT_BACKEND=1 STORAGE_VIZ_TRUSTED_PROXY=1 STORAGE_VIZ_ALLOWED_ORIGINS="$public_origin" STORAGE_VIZ_OPERATOR_ALLOWLIST="$operator" STORAGE_VIZ_SESSION_COOKIE_SECURE="$cookie_secure" \
    "$PYTHON" "$CANDIDATE_RELEASE/viewer/serve.py" &
  CANDIDATE_DASH_PID="$!"
  PYTHONDONTWRITEBYTECODE=1 STORAGE_VIZ_PROXY_BIND=127.0.0.1 STORAGE_VIZ_PROXY_PORT=1505 STORAGE_VIZ_PROXY_UPSTREAM_HOST=127.0.0.1 STORAGE_VIZ_PROXY_UPSTREAM_PORT=18088 STORAGE_VIZ_PROXY_OPERATOR="$operator" STORAGE_VIZ_PROXY_PUBLIC_ORIGIN="$public_origin" \
